pipeline {
  ...
  environment {
    ...
    CONTAINER_NAME  = 'consort-course-service'
    DOCKER_NETWORK  = 'consort-infra_consort-network'
    LOGS_DIR        = '/app/logs/course-service'
    UPLOADS_DIR     = '/app/data/course-uploads'
      ...
      steps {
        sh '''
          ...
          docker run -d \
            ...
            -p ${APP_PORT}:${APP_PORT} \
            -v ${LOGS_DIR}:/app/logs/course-service \
            -v ${UPLOADS_DIR}:/app/media/uploads \
            --env-file ${ENV_FILE} \
            -e TZ=Asia/Tashkent \
            ...
            -e SPRING_DATA_REDIS_HOST=consort-redis-course-service \
